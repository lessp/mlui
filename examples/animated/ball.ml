...
module Model = struct
  ...
  let init () = {
    position = Animation.Animated.make (400.0, 300.0);
    current_time = 0.0;
  }
end
...
let view (model : Model.t) : Msg.t node =
  ...
  view
    ...
    [
      view
        ~style:
          Style.(Styles.ball |> with_transform (Translate {
            x = x -. 50.0;
            y = y -. 50.0
          } ))
        [];
...
let run () =
  let window =
    Window.make ~width:800 ~height:600 ~title:"Ball" ()
  in
