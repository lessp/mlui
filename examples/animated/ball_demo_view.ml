...
let update msg model =
  ...
  | Msg.Tick dt ->
      ...
      let new_ball_position = Animation.Animated.step new_time model.ball_position in
      ...
      let active_ripples =
        List.filter
          ...
          model.ripples
      in
      ({ ball_position = new_ball_position; current_time = new_time; ripples = active_ripples }, Cmd.none)
  | Msg.Click (x, y) ->
      (* Add new ripple *)
      let new_ripple =
        {
        ...
        (fun ripple ->
          ...
          view
            ~style:
              ...
              |> with_border ~color:(Color.make ~r:255 ~g:02 ~b:147 ~a:alpha ()) ~width:3.0)
            [])
        model.ripples
    @ [
