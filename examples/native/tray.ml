...
let view model =
  let status =
    if model.Model.window_visible then
      "Window is visible"
    else
      "Window is hidden"
  in
  ...
  view
    ~style:
      Style.(
        default |> with_padding 40 |> with_flex_direction Column
        |> with_align_items Center
        |> with_background Color.white)
    [
      text
        ~style:
          Style.(default |> with_font_size 32.0 |> with_text_color Color.black)
        "🔔 System Tray Demo";
      view ~style:Style.(default |> with_padding 20) [];
      text
        ~style:
          Style.(
            default |> with_font_size 18.0 |> with_text_color Color.dark_gray)
        status;
      view ~style:Style.(default |> with_padding 15) [];
      text
        ~style:
          Style.(default |> with_font_size 14.0 |> with_text_color Color.gray)
        "Click the tray icon to toggle window visibility";
    ]

let subscriptions model = Sub.Tray.on_click model.Model.tray Msg.TrayClicked
