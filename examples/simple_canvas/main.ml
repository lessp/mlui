...
let view (_ : model) : msg Mlui.node =
  view ~style:Styles.canvas
    [
      canvas
        ...
        [
          rectangle ~x:20.0 ~y:20.0 ~width:120.0 ~height:80.0
            ~style:(fill Color.blue);
          ellipse ~cx:220.0 ~cy:120.0 ~rx:50.0 ~ry:35.0
            ~style:(fill_and_stroke Color.yellow Color.black 3.0);
          path
            ~points:[ (40.0, 200.0); (80.0, 230.0); (140.0, 210.0) ]
            ~style:(stroke Color.red 4.0);
        ];
    ]

let run () =
  let window = Window.make ~width:400 ~height:300 () in
  Mlui.run ~window ~init:(init ()) ~update ~view ()
